...
function getDeviceFullName(device) {
	if (heimdallSettings.useZoneNames) {
		return getDeviceName(device) + " (" + device.zone.name + ")";
	}
	return getDeviceName(device);
}

function getDeviceFullSpokenName(device) {
	if (heimdallSettings.useZoneNames) {
		return device.name + " in " + device.zone.name;
	}
	return getDeviceName(device);
}
...
class Heimdall extends Homey.App {
    ...
    addDevice(device, api) {
        if (device.data.id === 'sMode') {
            sModeDevice = device;
            console.log('Found Mode Switch:          ' + getDeviceFullName(device))
            console.log('Variabele:                  ' + sModeDevice.name)
        }
        if (device.data.id === 'aMode') {
            aModeDevice = device;
            console.log('Found Alarm Button          ' + getDeviceFullName(device))
        ...
        if ('alarm_motion' in device.capabilities) {
            console.log('Found motion sensor:        ' + getDeviceFullName(device))
        ...
        if ('alarm_tamper' in device.capabilities) {
            console.log('Found tamper sensor:        ' + getDeviceFullName(device))
        ...
        if ('alarm_contact' in device.capabilities) {
            console.log('Found contact sensor:       ' + getDeviceFullName(device))
            ...
            if ( isMonitoredFull(device) ) {
                if ( sensorState ) {
                    ...
                    if ( sensorType == 'motion') {
                        this.alertSensorActive(value, nu, sensorType, Homey.__("atarming.warningMotion") + sensorStateReadable + Homey.__("atarming.on") + getDeviceFullName(device) + delayText)
                    } else if ( sensorType == 'contact') {
                        this.alertSensorActive(value, nu, sensorType, Homey.__("atarming.warningContact") + getDeviceFullName(device) + Homey.__("atarming.is") + sensorStateReadable + delayText) 
                    }
                }
            }
        }
...
function attachEventListener(device,sensorType) {
	...
	if (sensorType == 'motion') {
		device.on('alarm_motion', debounce(alarm_motion => { 
			stateChangeX(device,alarm_motion,sensorType)
		},250));
	} else if (sensorType == 'tamper') {
		device.on('alarm_tamper', debounce(alarm_tamper => { 
			stateChangeX(device,alarm_tamper,sensorType)
		},250));
	} else if (sensorType == 'contact') {
		device.on('alarm_contact', debounce(alarm_contact => { 
			stateChangeX(device,alarm_contact,sensorType)
		},250));
	}
    ...
    console.log('Attached Eventlistener:     ' + getDeviceFullName(device))
    if ( isMonitoredFull(device) ) {
        console.log('Fully Monitored device:     ' + getDeviceFullName(device))
    } 
    if ( isMonitoredPartial(device) ) {
        console.log('Partially Monitored device: ' + getDeviceFullName(device))
    }
    if ( isLogged(device) ) {
        console.log('Logged device:              ' + getDeviceFullName(device))
    }
}
...
function stateChangeX(device,alarm,sensorType) {
	stateChange(device, device.state, sensorType);
}
...
function stateChange(device,state,sensorType) {
    ...
    console.log('-----------------------------------------------')
    console.log('stateChange:            ' + getDeviceFullName(device))
    ...
    if ( sourceDeviceFull || sourceDevicePartial || sourceDeviceLog ) {
        ...
        logLine = color + nu + readableMode(surveillance) + " || Heimdall || " + getDeviceFullName(device) + " " + sensorType + ": " + sensorStateReadable;
        // Is sensorstate true?
        if ( sensorState ) {
            ...
            if ( !alarmCounterRunning && !alarm) {
                if ( ( surveillance == 'armed' && sourceDeviceFull ) || ( surveillance == 'partially_armed' && sourceDevicePartial ) ) {
                    ...
                    logLine = "al " + nu + readableMode(surveillance) + " || Heimdall || " + getDeviceFullSpokenName(device) + Homey.__("history.triggerdalarm")
                    if ( sensorType == 'motion' ) {
                        speak("motionTrue", getDeviceFullSpokenName(device) + " detected motion") 
                    }
                    if ( sensorType == 'contact' ) {
                        speak("doorOpen", getDeviceFullSpokenName(device) + " is opened") 
                    }         
                    if ( sensorType == 'tamper') {
                        speak("tamper", getDeviceFullSpokenName(device) + " detected tampering")
                    }         
                    if ( isDelayed(device) ) {   
                        ...
                        var tokens= { 'Reason': getDeviceFullName(device) + ': '+ sensorStateReadable , 'Duration': heimdallSettings.triggerDelay * 1 };
                ...
                if ( ( surveillance == 'armed' && sourceDeviceFull ) || ( surveillance == 'partially_armed' && sourceDevicePartial ) ) {
                    ...
                    logLine = color + nu + readableMode(surveillance) + " || Heimdall || " + getDeviceFullName(device) + ": " + sensorStateReadable + Homey.__("history.noalarmtriggercountdown");
                }
            } 
            else if ( alarm ) {
                ...
                if ( ( surveillance == 'armed' && sourceDeviceFull ) || ( surveillance == 'partially_armed' && sourceDevicePartial ) ) {
                    ...
                    logLine = color + nu + readableMode(surveillance) + " || Heimdall || " + getDeviceFullName(device) + ": " + sensorStateReadable + Homey.__("history.noalarmtriggeralarmstate");
                }
            }
        }
        let shouldLog = true;
        ...
        if (sourceDeviceLog) {
            ...
            var tokens = {'Device': getDeviceFullName(device), 'State': sensorStateReadable};
...
function activateAlarm(device,state,sensorState,nu,source) {
    ...
    if ( surveillance != 'disarmed' || source == "Flowcard" ) {
        ...
        if ( source == "Heimdall") {
            var tokens= {'Reason': getDeviceName(device) + ': '+ sensorState , 'Zone': device.zone.name };
            logLine = "al " + nu + readableMode(surveillance) + " || " + source + " || " + Homey.__("history.alarmactivated") + getDeviceFullName(device) + ": " + sensorState;
        } else {
